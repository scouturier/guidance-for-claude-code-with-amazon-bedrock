...
class BuildsCommand(Command):
    ...
    options = [
        ...
        option(
            "project",
            description="CodeBuild project name (default: auto-detect)",
            flag=False
        ),
        option(
            "status",
            description="Check status of a specific build by ID",
            flag=False
        ),
    ]
    ...
    def handle(self) -> int:
        ...
        try:
            ...
            console.print(f"  poetry run ccwb builds --status <build-id>")
            console.print("\n[dim]To start a new build:[/dim]")
            console.print(f"  poetry run ccwb package --target-platform windows")
